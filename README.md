...
* Зміни в білд Візардів (все поза папок `_Mriya` та `_RMC14`) варто уникати, за винятком .ftl файлів. Якщо це прототип – краще просто додайте рядки з потрібними вам значеннями до його ванільної версії та не забудьте додати відповідний коментар в обох випадках.
...
* Додавання нових прототипів/значень/систем повинно йти в білд Січі (все в папки `_Mriya` або створіть такі, якщо нема), а також їх айді повинні починатися на `MR`, наприклад: `MRKitSherpa` або `MR-plant`. У випадку, якщо це зони для мап використовуйте `MRSS` (Mriya Space Station), щоб вони зайвий раз не заважали в меню спавну об'єктів.
